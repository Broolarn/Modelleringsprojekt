...
public class Collision : MonoBehaviour {
    ...
    private static float v1A = -2.0f;
    private static float v1B = 1.0f;
	...
	void Update () {
        transform.position = new Vector3(transform.position.x + v1A/60.0f, 0.0f, 0.0f);
	}
    ...
    void onTriggerEnter(Collider other)
    {
        ...
        float v2A = v1A + Vector3.Dot(j, n) / mA;
        v1A = v2A;
        print(v1A);
